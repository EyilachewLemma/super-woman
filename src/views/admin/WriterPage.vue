<template>
  <div class="border rounded shadow-sm m-3 bg-white pb-1">
    <div class="d-flex px-3 pt-4 justify-content-between">
      ...
      <div>
        <button @click="addNewWriter()" class="btn addWriterBtn px-5">
          Add new Writer
        </button>
      </div>
    </div>
      ...
      <tbody>
        <tr v-for="(writer, index) in writers" :key="writer.id">
          ...
          <td
            class="py-2"
            :class="{
              activeWriter: writer.is_active,
              blockedWriter: !writer.is_active,
            }"
          >
            {{ isActive(writer.is_active) }}
          </td>
          <td>
            <div class="dropdown">
              <a
    ...
    <template #modalBody>
      <div class="mb-3" :class="{ warning: v$.writerData.first_name.$error }">
        ...
        <input
          ...
          class="form-control"
          id="fname"
          v-model="writerData.first_name"
        />
        <span class="error-msg mt-1">{{
          v$.writerData.first_name.$errors[0]?.$message
        }}</span>
      </div>
      <div class="mb-3" :class="{ warning: v$.writerData.last_name.$error }">
        ...
        <input
          ...
          class="form-control"
          id="lname"
          v-model="writerData.last_name"
        />
        <span class="error-msg mt-1">{{
          v$.writerData.last_name.$errors[0]?.$message
        }}</span>
      </div>
      <div class="mb-3" :class="{ warning: v$.writerData.phone_no.$error }">
        ...
        <input
          ...
          class="form-control"
          id="phone"
          v-model="writerData.phone_no"
        />
        <span class="error-msg mt-1">{{
          v$.writerData.phone_no.$errors[0]?.$message
        }}</span>
      </div>
      <div class="mb-3" :class="{ warning: v$.writerData.email.$error }">
        ...
        <input
          ...
          v-model="writerData.email"
        />
        <span class="error-msg mt-1">{{
          v$.writerData.email.$errors[0]?.$message
        }}</span>
      </div>
    </template>
  </base-modal>
  ...
  methods: {
      ...
      try {
        if (!this.v$.$error) {
          var response = await apiClient.put(
            "api/employees/" + this.writerData.id,
            this.writerData
          );
    ...
    async deleteWriter(writer) {
      ...
      var response = await apiClient.delete(
        `api/employees/${writer.id}?state=${state}`
      );
      if (response.status === 200) {
